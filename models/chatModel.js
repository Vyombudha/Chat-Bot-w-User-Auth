...
export async function create(email, chatName = "New Chat") {
    const uuid = uuidV4();
    const query = `insert into chats (uuid,chat_name,email) values (?,?, ?);`;
    try {
        const [result] = await pool.query(query, [uuid, chatName, email]);
...
/**
 * Update the chatName, recieved from ollama Stream , of the chat  to DB
 * @param {string} email - email of the user
 * @param {string} uuid - uuid of the chat generated by uuidV4() dynamically
 * @param {string} chatName 
 * @returns {object} {success : true , uuid - uuid of the chat} 
 * @throws Update Failed error, User not found error,  Generic Database error
 * @example inserterName("vyombudha7@gmail.com" , "@#2e132r31r" , "Chat ");
 */
export async function insertName(email, uuid, chatName) {
    const query = `update chats set chat_name=? where uuid=? and email=?;`;
    try {
        const [result] = await pool.query(query, [chatName, uuid, email]);
        if (result.affectedRows === 0) {
            throw new Error("Update failed - Chat not Found");
...
export async function deleteLatestPrompt(email, uuid) {
    const msgTableQuery = `DELETE FROM messages 
    WHERE uuid = ? 
    AND role = 'user'
    ...
    try {
        ...
        const [result] = await pool.query(msgTableQuery, [uuid]);
